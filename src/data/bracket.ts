import type { Match, MatchId, PickRoundKey, RoundConfig } from '../types';

export const pickRoundConfigs: RoundConfig[] = [
...
export const sectionHeaders: Record<string, PickRoundKey> = {
  'round of 32': 'round32',
  'round of 16': 'round16',
  'round of 8': 'round8',
  'round of 4': 'round4',
...
const formatMatchId = (prefix: 'r32' | 'r16' | 'qf' | 'sf', index: number): MatchId => `${prefix}-${String(index + 1).padStart(2, '0')}` as MatchId;

const makeRound32Matches = (): Match[] =>
...
const makeDerivedMatches = (prefix: 'r16' | 'qf' | 'sf', round: 'round16' | 'quarterfinal' | 'semifinal', sourceIds: MatchId[], label: string): Match[] =>
  Array.from({ length: sourceIds.length / 2 }, (_, index) => ({
...
export const matches: Match[] = [...round32Matches, ...round16Matches, ...quarterfinalMatches, ...semifinalMatches, finalMatch];
