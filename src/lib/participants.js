...
export const displayNameFromFileName = (fileName) => {
...
export const participantIdFromFileName = (fileName) => displayNameFromFileName(fileName)
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '');
const normalizeHeader = (line) => line.replace(/^#+/, '').trim().toLowerCase();
const formatTeam = (teamId) => teamsById[teamId]?.name ?? teamId;
const validateCounts = (participant) => {
    for (const config of pickRoundConfigs) {
        ...
        if (actualCount !== config.expectedCount) {
            throw new Error(`${participant.displayName}: ${config.label} expected ${config.expectedCount} entries but found ${actualCount}`);
        }
    }
};
const validateInitialRound = (participant) => {
    participant.picks.round32.forEach((teamId, index) => {
        if (teamId !== initialTeamIds[index]) {
            throw new Error(`${participant.displayName}: Round of 32 entry ${index + 1} expected ${formatTeam(initialTeamIds[index])} but found ${formatTeam(teamId)}`);
        }
    });
};
const validateAdvancementRound = (participant, sourceRound, targetRound) => {
    participant.picks[targetRound].forEach((teamId, index) => {
        ...
        if (!sourcePair.includes(teamId)) {
            throw new Error(`${participant.displayName}: ${pickRoundConfigs.find((round) => round.key === targetRound)?.label} entry ${index + 1} picked ${formatTeam(teamId)}, but expected one of ${sourcePair.map(formatTeam).join(' or ')}`);
        }
    });
};
...
export const parseParticipantFile = ({ fileName, text }) => {
